...
const HomeBanner = () => {
    ...
    const handleImageUpload =(e)=>{
      ...
      axios.post('http://localhost:8500/uploadBanner',formData)
      .then(res=>{
        if(res){
          Swal.fire({
            position: "top-center",
      ...
      const handleImageDelete =(event,id)=>{
        ...
        swalWithBootstrapButtons.fire({
          ...
          if (result.isConfirmed) {
            ...
            axios.delete(`http://localhost:8500/bannerImage/${id}`)
            .then(result=>{
    ...
    const handleFrom =(e)=>{
        ...
        const titleSecond = form.titleSecond.value
        const titleColorSecond = form.titleColorSecond.value
        const description = form.description.value
        const user = {subTitle,title,titleColor,titleSecond,titleColorSecond,description}
        // console.log(user)

        fetch('http://localhost:8500/addUser', {
            method:"POST",
            headers:{
        ...
        .then(data =>{
            ...
            Swal.fire({
              position: 'top-center',
              icon: 'success',
              title: ' update home banner section please reload',
              showConfirmButton: false,
              timer: 2000
            })
            // window.location.reload()
    ...
    const handleDelete =(event,id)=>{
        console.log('delete')
        fetch(`http://localhost:8500/banner/${id}`, {
            method:"DELETE",
        })
        .then(res =>res.json())
        .then(result =>{
  ...
  return (
    <div>
      <div className='px-3'>
        <h3 className='bg-indigo-500 rounded-md text-white text-2xl py-2 px-3'>Home Banner Text Edit:</h3>
        <div className='px-4'>
        ...
        <div className='w-6/12 py-5  shadow-lg mx-auto px-5 flex justify-center mt-3'>
            <form action="" onSubmit={handleFrom} className='w-full '>
                ...
                <div className='w-full flex mt-3 items-center gap-3'>
                    <label htmlFor="" className='text-base capitalize'>title:</label>
                    <input type="text" name='titleColor' className='w-full bg-transparent border border-indigo-500 px-2 rounded py-2 mt-1' placeholder='write a title first color text' />
                    <input type="text" name='titleSecond' className='w-full bg-transparent border border-indigo-500 px-2 rounded py-2 mt-1' placeholder='write a title second text' />
                    <input type="text" name='titleColorSecond' className='w-full bg-transparent border border-indigo-500 px-2 rounded py-2 mt-1' placeholder='write a title last color text' />
                </div>
                <div className='w-full'>
            ...
            <div className=''>
                {
                    social.map((item)=>(
                        <div key={item._id} className={`flex items-center bg-slate-300 px-4 py-3 rounded  gap-4 mt-3`}>
                            ...
                            <h3 className='w-3/12'>Title: {item.title} {item.titleColor} {item.titleSecond} {item.titleColorSecond}</h3>
